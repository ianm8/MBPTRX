...
namespace DSP
{
  ...
  static const int16_t __not_in_flash_func(process_ssb_ORG)(const int16_t in_i,const int16_t in_q,const uint32_t jnr_level,const uint8_t bw)
  {
    // remove DC
    ...
    // HPF
    const float audio_raw = FILTER::hpf_300f(audio_lpf);
  ...
  static const int16_t __not_in_flash_func(process_ssb_LPF)(const int16_t in_i,const int16_t in_q,const uint32_t jnr_level,const uint8_t bw)
  {
    // remove DC
    const float ii_ac = FILTER::dc1f((float)in_i / 32768.0f);
    const float qq_ac = FILTER::dc2f((float)in_q / 32768.0f);

    // remove first image
    const float ii = FILTER::lpf_fs4f_rx1(ii_ac);
    const float qq = FILTER::lpf_fs4f_rx2(qq_ac);

    // 2 bit local oscillator 
    volatile static struct {uint32_t lo:2;} quad = {0};

    // reject image (quadrature LO at FS/4 and difference)
    // USB - left
    //const float ssb = ((float[]){qq,ii,-qq,-ii})[quad.lo++];

    // LSB - right
    const float ssb = ((float[]){ii,qq,-ii,-qq})[quad.lo++];
  ...
  static const int16_t __not_in_flash_func(process_ssb_FS4)(const int16_t in_i,const int16_t in_q,const uint32_t jnr_level,const uint8_t bw)
  {
    // LSB - left @FS/4 spectrum
    // USB - right @FS/4 spectrum
    ...
    // remove first image
    const float ii = FILTER::hpf_fs4f_rx1(ii_ac);
    const float qq = FILTER::hpf_fs4f_rx2(qq_ac);
  ...
  static const int16_t __not_in_flash_func(process_ssb_FS8)(const int16_t in_i,const int16_t in_q,const uint32_t jnr_level,const uint8_t bw)
  {
    // LSB - left spectrum
    // USB - right spectrum
    ...
    // remove first image
    const float ii = FILTER::hpf_fs8f_rx1(ii_ac);
    const float qq = FILTER::hpf_fs8f_rx2(qq_ac);

    // reject image (quadrature LO and difference)
    const uint8_t I_index = lo;
    const uint8_t Q_index = lo+2;
    const float loI = UTIL::s8[I_index];
    const float loQ = UTIL::s8[Q_index];
    const float ssb = ii*loI + qq*loQ;
    lo++;
